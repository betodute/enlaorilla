...
function CharLine({
  ...
  let wordIndex = -1;
  let spaceCount = 0; // <-- track which space we’re on
    ...
    <div className={className} aria-label={text}>
      {chars.map((ch, i) => {
        ...
        if (isSpace) {
          ...
          // For "EN LA ORILLA", the second space is the one before "ORILLA".
          // Render it as a break marker (no nbsp), which we control via CSS on mobile.
          if (spaceCount === 2) {
            prevWasSpace = true;
            return (
              <span
                key={`sp-${i}`}
                className="char char--space char--break"
                aria-hidden="true"
              />
            );
          }

          // First space: render a normal breaking space
          prevWasSpace = true;
          return (
            <span key={`sp-${i}`} className="char char--space" aria-hidden="true">
              {" "}
            </span>
          );
        }
